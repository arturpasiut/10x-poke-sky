    ...
    >
      ...
      >
        <div className="absolute inset-x-6 top-6 flex items-center justify-between text-sm text-white/70">
          <span data-testid={`pokemon-card-dex-${pokemon.pokemonId}`}>{dexNumber}</span>
          <div className="flex gap-2" data-testid={`pokemon-card-types-${pokemon.pokemonId}`}>
            {typeBadges.map((type) => (
              <span
                key={type.value}
                className={clsx(
        ...
        <div className="absolute inset-x-0 top-16 flex justify-center">
          <div className="relative h-40 w-40 overflow-visible">
            ...
            <img
              ...
              loading="lazy"
              data-testid={`pokemon-card-sprite-${pokemon.pokemonId}`}
            />
          </div>
        </div>
        ...
        <div className="relative mt-auto space-y-2 pt-44 pb-16">
          <h3
            ...
            data-testid={`pokemon-card-name-${pokemon.pokemonId}`}
          >
            {displayName}
          </h3>
